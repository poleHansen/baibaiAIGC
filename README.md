...
当前桌面端采用 `Tauri + Python` 双层架构。为了让最终用户安装后直接可用，不依赖本机 Python 环境，打包流程分成两步：

1. 先把 `scripts/app_service.py` 打包成独立的 `app_service.exe`
2. 再由 Tauri 把前端、Rust 壳层、Python 后端和只读资源一起打进 MSI 安装包
...
### 第一步：构建 Python 后端可执行文件

在仓库根目录执行：

```powershell
./scripts/build_app_service.ps1
```

默认会使用 `.venv/Scripts/python.exe`，并把产物输出到：

```text
app/bin/app_service.exe
```

如果你的 Python 不在默认位置，可以显式指定：

```powershell
./scripts/build_app_service.ps1 -PythonExe ".venv/Scripts/python.exe"
```
...
该命令会先构建前端，再执行 `tauri build`。当前配置会把以下资源打进安装包：
...
如果在 Windows 上执行 `tauri build` 时出现 Rust 编译阶段内存不足，可以改用低并发构建：

```powershell
cd app
npm run desktop:build:lowmem
```

该命令会把 Cargo 并发数降到 1，并减少 codegen 并发，通常更适合内存偏紧的机器。

### 安装版运行时目录

安装版不再依赖仓库根目录读写数据。

- 只读资源目录：安装包内的 resources 目录，用于读取 `prompts/`、`references/`、`scripts/`
- 用户数据目录：`%APPDATA%/BaibaiAIGC`

安装版生成和写入的数据会落在用户数据目录，例如：

- `%APPDATA%/BaibaiAIGC/config.json`
- `%APPDATA%/BaibaiAIGC/finish/aigc_records.json`
- `%APPDATA%/BaibaiAIGC/finish/intermediate/`
- `%APPDATA%/BaibaiAIGC/finish/web_exports/`

### 验证要点

建议在没有 Python 环境的 Windows 机器上验证：

1. 应用能正常启动
2. 可以读取 prompt 资源
3. 可以导入 `.txt` 或 `.docx`
4. 可以执行轮次处理并生成历史记录
5. 输出文件和记录都写到 `%APPDATA%/BaibaiAIGC`
